...
class TelnetScan(object):
        ...
        if self.flag:
            ...
            try:
                ...
                if ' timeout' in error_msg:
                    ...
                    self.logger.debug('{:<17}{:<7}{}'.format(ip, port,error_msg))
            else:
    ...
    def run(self):
        self.logger.info("[*] Start telnet port detect...")
        #初始化输入端口格式
        self.init_port()
        #初始化IP:Port格式与扫描线程
        self.init_thread()
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.thread_num) as executor:
                for ip_port in self.ip_port_list:
                    executor.submit(self.telnet_scan, ip_port)
