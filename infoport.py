...
def main():
    ...
    from modules.port_masscan import MasscanScan
    from modules.port_tcpasyc import AsyncTcpScan
    from modules.port_nmap import NmapScan
    from modules.port_http import HttpScan
    from modules.port_golang import goScanPort
    #端口扫描模块添加第1步修改
    from modules.port_telnet import TelnetScan
    
    from modules.service_nmap import NmapGetPortService
    from modules.service_tcp import TcpGetPortService
...
#############################
    ...
    if config.scantype:
        ...
        #增加c1,c2,c3参数
        if 'c1' in config.scantype:
            config.scantype = 't1,t2'
        if 'c2' in config.scantype:
            config.scantype = 't1,t2,t3'
        if 'c3' in config.scantype:
            config.scantype = 't1,t2,t3,t4'
        ...
        for scantype in config.scantype.split(','):
            ...
            elif ("telnet" in scantype) or ("t4" in scantype) :
                #端口扫描模块添加第4步修改
                t_scan = TelnetScan(config)
                #返回的端口号类型是数字
                open_port_dict_telnet = t_scan.run()
                
            elif ("http" in scantype) or ("t5" in scantype) :
                h_scan = HttpScan(config)
                #返回的端口号类型是数字
                open_port_dict_http = h_scan.run()
                
            elif ("nmap" in scantype) or ("t6" in scantype) :
